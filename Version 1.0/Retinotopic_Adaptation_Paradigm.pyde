...
limit_n_training_trials = 20 #how many trials until either excluding a participants or skipping training
minimum_performance = 9 #the number of trials out of window_size trials that must be correct, divide minimum_performance by window_size to get a percentage 
window_size = 10 #the size of the moving value in which to measure correct trials
exclude_p = True #whether to exclude participants if they failed the training phase
...
def moving_window(prompts, results, window_size):
    score = 0
    if len(prompts) < window_size:
        return score
    else:
        for trials in range(len(prompts) - window_size, len(prompts)):
            if prompts[trials] == results[trials]:
                score += 1
        return score
...
def draw():
    ...
    global n_training_trials
    global training_performance
    global prompts_training
...
##------Training
        ...
        if sequence == "starting training":
            ...
            if keyPressed and millis() - timeinms > 500:
                ...
                txt.training_instructions_display()
                
        if sequence == "Training":
            
            print(n_training_trials)
            print(response_request.prompt_list_training, response_request.response_list_training)
            training_performance = moving_window(response_request.prompt_list_training, response_request.response_list_training, window_size)
                ...
                if repositioned == False:
                    ...
                    if repositioned == True and response_request.awaiting_response == False:
                        ...
                        if response_request.awaiting_response == True:
                            response_request.get_response_training(fixation_dot.xpos, fixation_dot.ypos, Collision.overlap)
                            if response_request.awaiting_response == False:
                                delaysequence = "wait"
                                n_training_trials += 1
                                timeinms = millis()
                                if training_performance >= minimum_performance or n_training_trials > limit_n_training_trials:
                                    if exclude_p == True:
                                        sequence = "save responses"
                                        print("The time of the training trials was " + str(millis()))
                                    if exclude_p == False:
...
##------DEBRIEF   
        ...
        if sequence == "save responses":
            ...
            file = open("scores_training.csv", "a")
            
            for ra, rb in zip(response_request.prompt_list_training, response_request.response_list_training):
                print(ra + "," + rb + ",")
                file.write(ra + "," + rb + ",")
